...
def get_training(s_num):
	...
	for t in trans_lens:
		...
		features = np.append(features, s2_features)
...
def get_features(audio_path):
	...
	S_split = np.append(Sh_flat, Sp_flat)
	features = np.append(S_flat, S_split)

	#croma
	C = librosa.feature.chroma_cqt(y=y_harmonic, sr=sr)
	C_flat = C.flatten()

	features = np.append(features, C_flat)
	...
	features = np.append(features, m_flat)
