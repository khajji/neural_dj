...
do
	...
	do
		...
		elif [[ $item == "s1"*".mp3"* ]]; then
			c1="$d$i/$item $dfn/song$i""1.mp3"
		elif [[ $item == "s2"*".mp3"* ]]; then
			c1="$d$i/$item $dfn/song$i""2.mp3"
		elif [[ $item == "podcast_$i.mp3" ]]; then
			c1="$d$i/$item $dfn/transition$i.mp3"
		else
			continue
		fi
